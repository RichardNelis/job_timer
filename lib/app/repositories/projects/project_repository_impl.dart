...
class ProjectRepositoryImpl implements IProjectRepository {
  ...
  ProjectRepositoryImpl({required IDatabase database}) : _database = database;
  ...
  Future<List<Project>> findByStatus(ProjectStatus status) async {
    ...
    final projects =
        connection.projects.filter().statusEqualTo(status).findAll();
  ...
  Future<void> finish(int projectId) async {
    try {
      ...
      await connection.writeTxn((isar) {
        return connection.projects.put(project, saveLinks: true);
      });
    } on IsarError catch (e, s) {
